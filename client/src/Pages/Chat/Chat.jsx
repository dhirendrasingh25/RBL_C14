...
const Chat = () => {
  ...
  // console.log(id.id);
  const { data: Allmessages, isLoading } = useGetMessageQuery({
    rid: user?._id,
    sid: id.id,
  })
  ...
  return (
    <div className="flex justify-between flex-col h-full">
      <div className="text-center text-xl font-medium border p-2 rounded-lg">
        ...
        <p>
          Location: {getProfile?.user?.location}, Email:{' '}
      ...
      <div>
        <div className="h-[558px] rounded-lg my-2 overflow-y-scroll">
          {messages.map((msg, index) => (
            <div
              key={index}
              className={`m-2 ${msg.sender == user._id ? 'flex flex-row-reverse sm:ml-72 ml-12' : ''}`}
            >
              <div className="py-2 px-4 w-fit bg-slate-100 rounded-lg">
                <strong>
                  {msg.sender == user._id ? 'You' : getProfile?.user?.name}
                </strong>
                <p>{msg.message}</p>
              </div>
            </div>
        ...
        <div className="w-full p-2 flex border rounded-lg flex-row space-x-4">
          ...
          <button
            ...
            type="submit"
            className="bg-blue-500 hover:bg-blue-300 text-white px-6 py-1 rounded-lg"
          >
            Send
          </button>
        </div>
